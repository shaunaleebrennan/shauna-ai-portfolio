...
import {profiles,buyingRoles,goals,dims,esc,analyse,react,verdict,evidenceStatus,rewriteBrief,summary} from "./it-engine.js?v=3.1";
...
function render(r){
  ...
  $('#verdict-tag').textContent=r.rule.label;
  $('#verdict-copy').textContent=r.audit.criticalClaims.length?`Critical claim to check: “${r.audit.criticalClaims[0].quote}” The numeric score does not clear this claim.`:'Scored against the job of this asset. Check the reasoning and sources before using the message.';
  $('#reaction-label').textContent='Gut Reaction';
  $('#reaction').textContent=react(r);
  $('#priorities').innerHTML=r.ranked.length?r.ranked.slice(0,3).map(id=>{const x=r.audit.rows[id];return `<li><b>${esc(dims[id].name)}</b><span>${esc(x.next)}</span><small class="points-note">${points(x.available)} points not yet earned · ${esc(x.reason)}</small></li>`}).join(''):'<li>All structural checks are met. Have a reviewer confirm the evidence and audience fit.</li>';
  const ids=[...r.ranked,...Object.keys(dims)].filter((id,i,a)=>a.indexOf(id)===i).slice(0,3);
  $('#questions').innerHTML=ids.map(id=>`<li>${esc(r.profile.questions[id])}</li>`).join('');
