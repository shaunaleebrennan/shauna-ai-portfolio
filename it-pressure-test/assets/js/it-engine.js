...
function react(r) {
  ...
  const role={owner:'I need to defend this decision.',champion:'I need to make the case internally.',influencer:'I need to check the practical requirements.'}[r.buyingRole]||'I need to assess this.';
  if(!gaps.length) return `${role} The structure gives me a starting point; I would check whether the claims and evidence apply to this exact offer.`;
  return `${role} My first question: ${r.profile.questions[gaps[0]]}${gaps[1]?` Then: ${r.profile.questions[gaps[1]]}`:''}`;
}
